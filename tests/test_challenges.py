...
def test_preorder_values_handles_right_heavy_tree() -> None:
    root = TreeNode("A", None, TreeNode("B", None, TreeNode("C")))
    assert preorder_values(root) == ["A", "B", "C"]


def test_inorder_values_handles_right_heavy_tree() -> None:
    root = TreeNode("A", None, TreeNode("B", None, TreeNode("C")))
    assert inorder_values(root) == ["A", "B", "C"]


def test_postorder_values_handles_right_heavy_tree() -> None:
    root = TreeNode("A", None, TreeNode("B", None, TreeNode("C")))
    assert postorder_values(root) == ["C", "B", "A"]


def test_preorder_values_handles_balanced_tree() -> None:
    root = TreeNode(
        "B",
        TreeNode("A"),
        TreeNode("C"),
    )
    assert preorder_values(root) == ["B", "A", "C"]


def test_inorder_values_handles_balanced_tree() -> None:
    root = TreeNode(
        "B",
        TreeNode("A"),
        TreeNode("C"),
...
def test_postorder_values_handles_balanced_tree() -> None:
    root = TreeNode(
        "B",
        TreeNode("A"),
        TreeNode("C"),
    )
    assert postorder_values(root) == ["A", "C", "B"]


def test_bst_contains_single_node_tree() -> None:
    root = TreeNode(10)
    assert bst_contains(root, 10) is True
    assert bst_contains(root, 5) is False


def test_bst_insert_multiple_values_and_structure() -> None:
    root = None
    values = [15, 10, 20, 8, 12, 17, 25]
    for v in values:
        root = bst_insert(root, v)

    assert inorder_values(root) == [8, 10, 12, 15, 17, 20, 25]


def test_bst_insert_left_skewed_tree() -> None:
    root = None
    for v in [5, 4, 3, 2, 1]:
        root = bst_insert(root, v)

    assert inorder_values(root) == [1, 2, 3, 4, 5]


def test_bst_insert_right_skewed_tree() -> None:
    root = None
    for v in [1, 2, 3, 4, 5]:
        root = bst_insert(root, v)
...
def test_bst_insert_and_contains_combined() -> None:
    root = None
    for v in [10, 5, 15, 3, 7]:
        root = bst_insert(root, v)

    assert bst_contains(root, 7) is True
    assert bst_contains(root, 99) is False


def test_bst_insert_does_not_modify_structure_on_duplicate() -> None:
    root = build_station_bst()
    original = inorder_values(root)
    updated = bst_insert(root, 40)  # duplicate root

    assert inorder_values(updated) == original